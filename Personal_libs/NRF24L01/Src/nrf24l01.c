...
#include "u_delay.h"
#include "nrf24l01.h"
#include "main.h"   // para NRF_CE_Pin, NRF_CSN_Pin, etc.

static delUs_t del_ce;		//Para probar lo puse en modo bloqueantre
...
uint8_t nrfReadReg(uint8_t reg) {
    ...
    while (_busy);          // esperás que el DMA termine
    	return _rx_buf[1];      // _rx_buf[0] es el STATUS, [1] es el dato. MODIFICAR A NO BLOQUEANTE !!!!!!!!!!!!!!!!!!!!!!!!!!!!
  }

void nrfInit(SPI_HandleTypeDef *hspi) {			//Inicialización del driver.
...
void nrfSetModeTX(uint8_t *address, uint8_t channel, uint8_t payload_len) {
    ceLow();
    ...
    while(_busy);
    nrfWriteAddr(NRF_TX_ADDR,   address, 5);
    while(_busy);
    nrfWriteAddr(NRF_RX_ADDR_P0, address, 5);  // igual a TX_ADDR para ACK
    while(_busy);
...
void nrfWriteAddr(uint8_t reg, uint8_t *addr, uint8_t len) {
    _tx_buf[0] = NRF_CMD_W_REG | reg;
...
void nrfTransmit(uint8_t *data, uint8_t len) {
    ...
    ceHigh();
     delayUsInit(&del_ce, 10);
     while(!delayUsRead(&del_ce));
     ceLow();
}
